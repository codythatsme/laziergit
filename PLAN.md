...
- **M4 — Bundled extensions.** The eight, roughly status → files → branches → commits → stash → diff → commit-flow → sync, each stress-feeding the API (selection model, decorations, menus, multi-pane focus). API changes surface here — make them, don't work around them; the two already known are the unborn-HEAD and gone-upstream encodings in [extension-api.md §5.12](./docs/extension-api.md). *Done when: the everyday loop works end-to-end on this repo.*
