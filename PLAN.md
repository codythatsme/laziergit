...
- **M5 — Acceptance & polish.** Run the acceptance test cold, fix every friction it finds, sweep the visual design pass (opencode-grade aesthetics: theme tokens, spacing, borders, empty states). *Done when: the acceptance test passes and lazygit is uninstalled.*
  ...
  **The acceptance extension now also ships.** `gh-workflows` is a Bundled Extension — the daily driver wants the Actions pane out of the box, not re-authored into every config directory. The bundled copy has since grown well past §2 (see the tenth polish item below); §2 stays the minimal worked example, and the divergences bundling originally demanded still hold: a machine without `gh` renders the spawn failure instead of leaking an unhandled rejection, and the placement hint sits below stash rather than tying with it. §2 itself is unchanged, and its e2e now also proves scope precedence — the sandboxed global install shadows the bundled copy, so the acceptance test still measures a user file against a distribution that already contains the same name. Because every session boots the pane, every e2e sandbox carries a `gh` stub answering an empty run list; the screen must never depend on the machine's own gh, its auth state, or the network.
  ...
  **A ninth: targeted history rewriting.** The commits menu now carries lazygit's three daily rebase operations: squash a commit into its first parent, reword it in the existing multiline commit editor, and drop it. They are deliberately narrower than a rebase-todo UI: only non-merge commits on the checked-out branch's first-parent chain are offered, the working tree must be clean, and a replay conflict is automatically aborted so the original history comes back intact. Git still gets `--rebase-merges`, `--keep-empty`, and a sequence editor supplied as an argv-safe Bun command; `rebase.updateRefs` is pinned off so rewriting the checked-out branch cannot silently move another local branch. The public seams grew only where both operations proved they had to: `RawOptions.env` for per-invocation Git editors, and message-only amend support so rewording cannot fold a staged index into the selected commit.

  **A tenth: the Actions pane became a daily driver.** The bundled `gh-workflows` grew lazygit's drill-in shape without any new core seam: one pane, three faces held in a `createCell` view state — `return` descends runs → a run's jobs (steps folded under each job, failed and live jobs starting open) → a failed job's log tail, `escape` climbs back out. GitHub reruns whole jobs, never steps, so `r` retries the selected job (`--failed`-only at the run level when the run failed, `shift+r` for everything), `x` cancels behind a confirm, `a` widens the list from the current branch to every branch, and `/` filters. The pane polls `gh` only while something is live and goes quiet when the list settles; a rerun or cancel in flight replaces the row's status glyph with its verb, keyed re-entrancy so a held key cannot fire twice. The row-building — fold defaults, flattening, cursor re-anchoring keys, durations, log tailing — is a pure `model.ts` mirroring `files/tree.ts`, unit-tested beside the pane's own bundled-kernel tests.
