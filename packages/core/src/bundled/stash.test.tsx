...
describe("stash pane", () => {
  it("renders a row per entry, newest first, with its branch and age", async () => {
    ...
    expect(rendered).toContain("stash@{0} wip two on main now")
    expect(rendered).toContain("stash@{1} wip one on main now")
  })

  it("says there are no stashes rather than rendering an empty box", async () => {
    const harness = await stashHarness()
    await start(harness)

    expect(frame(harness)).toContain("no stashes")
  })

  it("shows the selected stash in the diff pane, and only once focused", async () => {
    const harness = await stashHarness()
    await stash(harness, "wip one")
    await stash(harness, "wip two")
    await start(harness)

    // Another Pane holds focus, so the stash selection is not the diff's business yet.
    expect(frame(harness)).toContain("diff: nothing")

    await press(harness, () => harness.setup.mockInput.pressKey("5"))
    expect(frame(harness)).toContain("diff: stash stash@{0}")

    await press(harness, () => harness.setup.mockInput.pressKey("j"))
    expect(frame(harness)).toContain("diff: stash stash@{1}")
  })

  it("scrolls the cursor into view rather than growing past the pane", async () => {
    // Twelve rows in a Pane that shows fewer: the last entry cannot be on screen at rest.
    const harness = await stashHarness(16)
    for (let entry = 0; entry < 12; entry += 1) await stash(harness, `wip ${entry}`)
    await start(harness)

    // Newest first, so the list reads stash@{0} "wip 11" down to stash@{11} "wip 0".
    const before = frame(harness)
    expect(before).toContain("stash@{0} wip 11")
    expect(before).not.toContain("stash@{11} wip 0")

    await press(harness, () => harness.setup.mockInput.pressKey("5"))
    await press(harness, () => harness.setup.mockInput.pressKey("G"))

    const after = frame(harness)
    expect(after).toContain("stash@{11} wip 0")
    // Scrolled, not merely grown: the top of the list has gone out of view, which is also
    // what keeps the box from painting over the Pane's header.
    expect(after).not.toContain("stash@{0} wip 11")
  })

  it("stops naming a selection once the pane is no longer on screen", async () => {
    const harness = await stashHarness()
    await stash(harness, "wip one")
    await stash(harness, "wip two")
    // One cell holding both Panes, so focusing the files Pane hides — and unmounts — this one.
    await start(harness, `{ "layout": { "columns": [[["stash", "files"]], ["diff"]] } }`)

    await press(harness, () => harness.setup.mockInput.pressKey("5"))
    await press(harness, () => harness.setup.mockInput.pressKey("R"))
    expect(frame(harness)).toContain("selection is wip two")
