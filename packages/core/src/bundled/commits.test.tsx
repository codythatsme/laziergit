...
/**
 * A long history, cheaply: empty commits skip the write-and-add that {@link commit} does per
 * row, and this fixture exists to be *tall* rather than to contain anything.
 */
async function commitMany(repo: Repo, count: number): Promise<void> {
  for (let index = 1; index <= count; index += 1) {
    await repo.run("commit", "--quiet", "--allow-empty", "--message", `commit ${index}`)
  }
}
...
describe("the commits Pane", () => {
  it("renders a row per commit and marks the merges", async () => {
    ...
    expect(rendered).toContain(await repo.shortOid("HEAD"))
    for (const subject of ["merge feature", "main work", "feature work", "first commit"]) {
      expect(rendered).toContain(subject)
    }
    // The author, and an age — a log that cannot say who or when is half a log.
    expect(rendered).toContain("Ada Lovelace")
    expect(rendered).toContain("now")
    // Exactly one merge in this history, so exactly one marker: a marker on every row would
    // pass a "contains" assertion while telling the reader nothing.
    expect(rendered.split("⑂").length - 1).toBe(1)
  })

  it("says a repository with no commits has none yet, rather than drawing an empty box", async () => {
    const repo = await openRepo()

    await renderApp(repo.harness)

    // An unborn HEAD has no oid to read, and the Pane says so instead of rendering nothing.
    expect(frame(repo.harness)).toContain("no commits yet")
  })

  it("says there is no repository outside one, instead of promising a first commit", async () => {
    const repo = await openRepo({ git: false })

    await renderApp(repo.harness)

    // Both cases arrive as an unborn HEAD, and only the empty branch name tells them apart.
    // Promising a first commit in a directory git has never heard of is the lie under test.
    expect(frame(repo.harness)).toContain("no repository")
    expect(frame(repo.harness)).not.toContain("no commits yet")
  })

  it("scrolls to the far end of a full commit window instead of clipping at the first screenful", async () => {
    const repo = await openRepo()
    // The default `git.commitLimit`, which is the tallest list this Pane is ever asked to draw.
    await commitMany(repo, 200)

    await renderApp(repo.harness)
    // HEAD is newest-first, so the far end of the window is the first commit ever made. The
    // trailing space matters: without it "commit 1" also matches "commit 100".
    expect(frame(repo.harness)).toContain("commit 200")
    expect(frame(repo.harness)).not.toContain("commit 1 ")

    await press(repo.harness, "4")
    await press(repo.harness, "G")

    // The row every key now acts on is on screen, 199 rows below where the window started...
    expect(frame(repo.harness)).toContain("commit 1 ")
    // ...and the top of the log is genuinely gone, so the box scrolled rather than grew.
    expect(frame(repo.harness)).not.toContain("commit 200")
  }, 60_000)

  it("names the limit when the cursor reaches the end of the loaded window", async () => {
    const repo = await openRepo({ config: layout(`, "git": { "commitLimit": 2 }`) })
    await commit(repo, "first commit")
    await commit(repo, "second commit")
    await commit(repo, "third commit")

    await renderApp(repo.harness)
    // Two loaded of three: the third is past the window and must not appear.
    expect(frame(repo.harness)).not.toContain("first commit")
    expect(frame(repo.harness)).not.toContain("commitLimit")

    await press(repo.harness, "4")
    await press(repo.harness, "G")

    expect(frame(repo.harness)).toContain("2 shown; raise git.commitLimit for more")
  })

  it("leaves the footer off when the window really is the whole history", async () => {
    const repo = await openRepo()
    await commit(repo, "first commit")
    await commit(repo, "second commit")
    ...
    // The last row is the root commit, so there is nothing past it to promise.
    expect(frame(repo.harness)).toContain("first commit")
    expect(frame(repo.harness)).not.toContain("commitLimit")
  })
})

describe("the commits Pane and the diff", () => {
  it("shows the selected commit while focused, and does not steal the diff while it is not", async () => {
    const repo = await openRepo()
    await commit(repo, "first commit")
    await commit(repo, "second commit")

    await renderApp(repo.harness)
    // The diff Pane holds focus, so nothing has been selected in the commits Pane yet.
    expect(frame(repo.harness)).toContain("diff: nothing")

    await press(repo.harness, "4")
    const head = await repo.oid("HEAD")
    await waitForFrame(repo.harness, `diff: commit ${head.slice(0, 7)} path=none`)

    await press(repo.harness, "j")
    const parent = await repo.oid("HEAD~1")
    await waitForFrame(repo.harness, `diff: commit ${parent.slice(0, 7)} path=none`)
  })
})
